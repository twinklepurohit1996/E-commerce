...
import { createUser, findAndUpdate, findUser, deleteuser, findU } from '../services/userService'
...
const login = async (req: Request, res: Response) => {
    if (!req.body.email) {
        ...
        const user = await findUser({ email: req.body.email, role: 'Administrator' });
        console.log(user)
        if (user) {
...
export { login , registration , userList , deleteUser , editUser , updateUser , userProfile}
